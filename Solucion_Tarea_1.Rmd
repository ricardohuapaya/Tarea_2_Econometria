...
Haciendo un analisis de correlacion entre la variable ``PACKS`` y cada una de sus variables explicativas nos da un resultado similar a la intuición grafica. El resultado matematico indica que existe una correlación negativa debil entre ingreso y la cantidad de cigarros consumidos.
...
La prueba $f$ conjunta nos devuelve un P-value de ****, entonces con un nivel de significancia del 5% se ----- la hipotesis nula conjunta de que el precio y el ingreso
...
```
Cuando las variables explicativas toman el valor de 0 los costos son negativos. -----Cabe resaltar que las tres primeras variables explicativas tienen rendimientos decrecientes luego la cuarta variable de combustible  presenta redimientos crecientes a escala.-------- eliminar

### 2.3 Pruebas t
Utilizando la misma función que en la pregunta anterior, y con la función summary se obtiene la información necesaria para obtener los resultados de la prueba $t$.
```{r}
summary(linear.energy)
```


Tomando $H_{0}:  \beta_{1}=0$ (Producción no tiene efecto en los Costos) La prueba $t$ realizada para la producción nos devuelve un P-value de 2e-16 (0.00000000000000022), entonces con un nivel de significancia del 5% podemos rechazar la hipotesis nula donde la producción no afecta los costos. 
...
Tomando $H_{0}:  \beta_{4}=0$  (Combustible no tiene efecto en los Costos) La prueba $t$ realizada para la producción nos devuelve un P-value de 6.48e-07 (0.000000648), entonces con un nivel de significancia del 5% podemos rechazar la hipotesis nula donde la producción no afecta los costos.
...
La prueba $f$ conjunta nos devuelve un P-value de 2.2e-16 (0.00000000000000022000000000000002), entonces con un nivel de significancia del 5% se rechaza la hipotesis nula conjunta de que los B´s no afectan los costos.
...
$$
GDP = \beta_{0}+\beta_{1}ln(energy)+\mu
$$

...
$$
ln(PACKS) = \beta_{0}+\beta_{1}income+\mu
$$

A pesar de que el ingreso es una variable que pueda explicarnos a cierto nivel la demanda por cigarillos, en este caso no es estadisticamente significativo y solo nos genera suciedad en la regresión. Adicionalmente en un sentido economico, la demanda de los cigarillos no tienden a ser influeciados por el ingreso que los individuos tienen, por su naturaleza son un bien "necesario" que se cosume irrelevantemente del ingreso de las personas.

```{r, warning = FALSE}
linear.cigs.corr <-lm(PACKS ~ income, data=cigs)
```
...
El cambio en la regresión resulta en un incremento enorme en la dispercion de las perturbaciones estocasticas de nuestra regresión.
```{r, warning = FALSE}
whitecig2=as.data.frame(white_lm(linear.cigs.corr))
stargazer(whitecigs2, type ="text",title = "Cuadro #2.1: Prueba de Heterocedastidad", summary = FALSE)
```
Observando el resultado se obtiene que con un nivel de significancia de 5%  una vez más se rechaza la hipotesis nula, confirmando que existe heteroscedasticidad en el modelo corregido.

Esto nos dice que en la nueva regresión se observan estimadores insesgados pero que no son los mejores estimadores linealmente independientes y cuyos errores estándar son incorrectos. Utilizamos la correción de White donde el error estándar asociado con el coefficiente de Price, el estadistico t y el pvalue se ven afectados. Sin embargo no cambia de manera significativa el efecto de Price en explicar la regresión.

```{r, warning = FALSE}
linear.cigs.w=coeftest(linear.cigs.corr,vcov.=vcovHC(linear.cigs.corr,"HC1"))
```

### 1.6 Comparación

```{r}
stargazer(modelo1,linear.cigs.corr,linear.cigs.w,type="text", title="Cuadro #2.2: Original vs Corregidos")
```
...
Se procede a realizar la correción por heterocedasticidad donde se toma el logaritmo de los residuos al cuadrado de la regresión.
Luego cada variable explicativa se resta con este componente y se corre la regresión con los nuevos datos
```{r}

datos_reg$u2 <- log(residuals(linear.energy)^2)


datos_reg$Costo_2 <- datos_reg$Costo - (u2)
datos_reg$Produccion_2 <- datos_reg$Produccion - (u2)
datos_reg$Trabajo_2 <- datos_reg$Trabajo - (u2)
datos_reg$Capital_2 <- datos_reg$Capital - (u2)
datos_reg$Combustible_2 <- datos_reg$Combustible - (u2)

linear.energy.corre <- lm( Costo_2 ~  Produccion_2 + Trabajo_2 + Capital_2 + Combustible_2 , data= datos_reg)
...
X <- as.matrix(cbind(datos_reg$Produccion_2, datos_reg$Trabajo_2, datos_reg$Capital_2, datos_reg$Combustible_2))
...
Para el $\beta_{0}$ (el intercepto), se interpreta como el precio que existe al ser las variables explicativas iguales a cero. Se observa que, el precio es negativo cuando las variables explicativas son cero. No tiene mucho sentido económico, puesto que no hay casas en venta con precios negativos en el mercado. 

Para el $\beta_{1}$ se interpreta como el aporte que genera el incremento de un metro cuadrado de la variable "lotsize" al precio de las casas. Se ve que afecta positivamente al precio. 

Para el $\beta_{2}$ se interpreta como el aporte que genera el incremento de una unidad de la variable "bedrooms" (número de habitaciones) al precio de las casas. Igual que el tamaño del lote, afecta positivamente al precio. 

Para el $\beta_{3}$ se interpreta como el aporte que genera el incremento de una unidad de la variable "bathrooms" (número de baños) al precio de las casas. Se ve que, al igual que "lotsize" y "bedrooms" afecta positivamente al precio, pero, el número de baños afecta en una cuantía mayor al precio. Es decir (comparándolo con las habitaciones), es más caro un baño adicional que una habitación adicional.   
...
Tomando $H_{0}: \beta_{0}=0$ (El intercepto es nulo) La prueba $t$ realizada para el intercepto, nos devuelve un P-value que es aproximadamente, 0.523. Entonces con un nivel de significancia del 5% podemos no rechazar la hipotesis nula donde el intercepto no afecta los precios, puesto que el P-value es mayor que el nivel de tolerancia a cometer el error tipo I. Esto confirma la intuición del inciso pasado, en el que se decía que, no tendría mucho sentido que eso fuese cierto (que el precio de las casas fuesen negativos)

Tomando $H_{0}: \beta_{1}=0$ (El tamaño del lote no tiene efecto sobre el precio de las casas) La prueba $t$ realizada para el tamaño de los lotes nos devuelve un P-value muy cercano a cero. Entonces se puede rechazar la hipotesis nula. Es decir, el tamaño del lote sí tiene un efecto sobre el precio de las casas.

Tomando $H_{0}: \beta_{2}=0$ (El número de habitaciones no tiene un efecto en el precio de las casas). La prueba $t$ realizada para el número de habitaciones nos devuelve un P-value cercano a cero. Entonces con un nivel de significancia del 5% podemos rechazar la hipotesis nula donde el número de habitaciones no afectan los precios de las casas. Es decir, el número de habitaciones sí afecta al precio de las casas. 

Tomando $H_{0}: \beta_{3}=0$ (Número de baños no tiene efecto en los precios) La prueba $t$ realizada para el número de baños nos devuelve un P-value que tiende a 0. Entonces con un nivel de significancia del 5% podemos rechazar la hipotesis nula donde el número de baños no afecta los precios de las casas.
...
La prueba F conjunta nos devuelve un P-value que tiende a 0. Entonces dado lo anterior y con un nivel de significancia del 5% se rechaza la hipotesis nula de que los Betas de forma conjunta no afectan los precios de las casas.
...
Inicialmente considere dentro de nuestra correlación si ha de existir un problema de multicolinealidad perfecta. Corremos un código en el modelo para observar si este detecta algún error.
...
Visto los resultados de la matriz, vemos que sí existe una correlación entre varias variables. Por ello ahora usaremos el analisis del indice K.
...
Con el fin de remediar la violación de los supuestos, se procede de la siguiente manera. 

1. Se obtiene una nueva variable: la proporción de baños sobre habitaciones. Es probable esperar que, el número va a ser menor o igual que uno, puesto que, normalmente, se tienen más habitaciones que baños. 
...
4. Obtenemos la raíz cuadrada del tamaño de los lotes. 

Es decir: 

```{r, warning = FALSE}
house$babed_ratio <- house$bathrooms/house$bedrooms
house$log_babedratio <- log(house$babed_ratio)
house$log_PRICE <- log(house$PRICE)
house$sqrt_lotsize <- sqrt(house$lotsize)
```

A partir de lo anterior, corremos la siguiente regresión: 

```{r, warning = FALSE}
linear.house1 <- lm( formula = log_PRICE ~ sqrt_lotsize + log_babedratio, data = house)
```

```{r, echo=FALSE, warning=FALSE}
stargazer(linear.house1, type = "text", title = "Cuadro #7: Resultados de la Nueva Regresión")
coeflin = coefficients(linear.house1)
```
...
```{r Jarque-Bera 3, echo=FALSE}
jarque.bera.test(residuals(linear.house1)/50000)
```

Como ahora se obtiene un P-value mayor que un error de tolerancia de 5%, no se rechaza la hipótesis nula. Por lo tanto, podemos no rechazar el hecho de que existe homocedasticidad. 
...
stargazer(white.house1, type ="text",title = "Cuadro #8: Prueba de Heterocedastidad", summary = FALSE)

white.house1
```

De nuevo, vemos que el nuevo P-value es mayor que el error de tolerancia al 5%, lo que significa que no se rechaza la hipótesis nula. Por lo tanto, existe homocedasticidad. 

#### Corrigiendo la multicolinealidad. 

Aplicando nuevamente las pruebas llevadas a cabo anteriormente, se tiene que: 

Si buscamos la multicolinealidad perfecta de la nueva regresión, no la vamos a encontrar. De acuerdo con el siguiente código: 
...
X <- as.matrix(cbind(house$sqrt_lotsize, house$log_babedratio))
...
Ahora, vemos que, el IC sigue siendo mayor que 30, pero, es mucho menor que el IC de la regresión no transformada. 
...
stargazer(linear.house, linear.house1, type = "text", title = "Cuadro: Comparación de las Regresiones")

```

Y, ahora, gracias a las transformaciones que se le aplicaron a las variables explicativas y dependiente, se logra corregir las violaciones a los supuestos. En el caso de la multicolinealidad, si bien no se elimina por completo, se logra disminuir la dependencia entre las variables explicativas. 
